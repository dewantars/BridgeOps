<x-guest-layout>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-stack-md">
        ...
        <!-- Name -->
        <div>
            <label class="block font-body-sm text-body-sm font-bold text-on-surface mb-2" for="name">Nama Lengkap</label>
            <div class="relative">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input class="w-full pl-10 pr-4 py-3 bg-surface-container-low border border-transparent rounded-lg font-body-sm text-body-sm text-on-surface focus:bg-surface-container-lowest focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors" id="email" name="email" placeholder="nama@perusahaan.com" required="" type="email" value="{{ old('email') }}" autocomplete="username" />
            </div>
            @error('email')
                <p class="text-error text-xs mt-1">{{ $message }}</p>
            @enderror
        ...
        <!-- Confirm Password -->
        <div>
            <label class="block font-body-sm text-body-sm font-bold text-on-surface mb-2" for="password_confirmation">Konfirmasi Kata Sandi</label>
            <div class="relative">
                <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-outline" style="font-variation-settings: 'FILL' 0;">lock</span>
    ...
    <div class="mt-stack-lg text-center font-body-sm text-body-sm text-on-surface-variant border-t border-surface-container-high pt-stack-lg">
        Sudah memiliki akun? 
        <a class="font-semibold text-primary hover:underline hover:text-surface-tint transition-colors" href="{{ route('login') }}">Masuk sekarang</a>
    </div>
</x-guest-layout>
